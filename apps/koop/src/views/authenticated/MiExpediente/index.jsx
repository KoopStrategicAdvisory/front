    ...
    >
      <div className="dash-card" style={{ width: '100%', maxWidth: 1320 }}>
        ...
        <div className="me-layout">
          {/* Izquierda: Clientes asignados al admin (o carpetas de usuario) */}
          <aside className="me-left dash-item">
            ...
            <div className="me-tree">
                      ...
                      return (
                        <div key={folder.path} style={{ marginBottom: '8px' }}>
                          <div
                            ...
                            onClick={() => handleUserFolderAccordionClick(folder)}
                          >
                            <span style={{
              ...
              {/* Carpetas del cliente en modo modal */}
                        ...
                        >
                          ...
                          <td>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                              ...
                              >
                                <span role="img" aria-label="ver" style={{ fontSize: 12 }}>👁️</span>
                              ...
                              <button
                                ...
                                disabled={isAdmin && multiSelectMode}
                              >
                                <span role="img" aria-label="descargar" style={{ fontSize: 12 }}>⬇️</span>
                                <span>Descargar</span>
                              </button>
                              <button
                                type="button"
                                ...
                                disabled={isAdmin && multiSelectMode}
                              >
                                <span role="img" aria-label="link" style={{ fontSize: 12 }}>🔗</span>
                                <span>Compartir</span>
                              </button>
                            </div>
                          </td>
          ...
          {/* Derecha: Herramientas (modo modal) o Datos del Proceso (modo normal) */}
                ...
                <div className="me-right-content" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  {showFileNameInput && selectedFile && (
                    <div style={{
                      ...
                      <div style={{ fontSize: '12px', color: '#9fb3cc', marginBottom: '8px', fontWeight: '500' }}>
      ...
      {isModal && showCreateProcess && (
        ...
        >
          <div className="dash-card" style={{ maxWidth: '500px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dash-item" style={{ display: 'grid', gap: 16 }}>
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Nombre del Proceso:</label>
                <input
                  type="text"
                  className="me-input"
                ...
                />
                <div style={{ fontSize: '12px', color: '#fbbf24', backgroundColor: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '6px', padding: '8px 12px', marginTop: '8px' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Tipo de Proceso:</label>
                <select
                  className="me-select"
                  style={{ width: '100%' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button className="btn btn-secondary" onClick={() => { setShowCreateProcess(false); setNewProcessName(''); setNewProcessType(''); }} disabled={creatingProcess}>Cancelar</button>
              <button className="btn btn-primary" onClick={onCreateProcess} disabled={creatingProcess || !newProcessName.trim()}>
      ...
      {isModal && showRenameFile && selectedFileForAction && (
        ...
        >
          <div className="dash-card" style={{ maxWidth: '500px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dash-item" style={{ display: 'grid', gap: 16 }}>
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Archivo actual:</label>
                <div style={{ background: '#1e2a3a', padding: '8px 12px', borderRadius: '6px', color: '#9fb3cc', fontSize: '14px' }}>
                  {selectedFileForAction.name || selectedFileForAction.key?.split('/').pop()}
                </div>
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Nuevo nombre:</label>
                <input type="text" className="me-input" style={{ width: '100%' }} />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button className="btn btn-secondary" onClick={() => setShowRenameFile(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={() => setShowRenameFile(false)}>Renombrar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModal && showDeleteFile && selectedFileForAction && (
        ...
        >
          <div className="dash-card" style={{ maxWidth: '500px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dash-item" style={{ display: 'grid', gap: 16 }}>
              <div>¿Estás seguro de que quieres eliminar el archivo?</div>
              <div style={{ background: '#1e2a3a', padding: '12px', borderRadius: '6px', color: '#e5edf7', fontSize: '14px' }}>
                <strong>Archivo:</strong> {selectedFileForAction.name || selectedFileForAction.key?.split('/').pop()}
              </div>
              <div style={{ background: '#7f1d1d', color: '#fecaca', padding: '8px 12px', borderRadius: '6px', fontSize: '12px' }}>
                ⚠️ Esta acción no se puede deshacer
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button className="btn btn-secondary" onClick={() => setShowDeleteFile(false)}>Cancelar</button>
              <button className="btn btn-danger" onClick={() => setShowDeleteFile(false)}>Eliminar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModal && showProcessInfo && (
        ...
        >
          <div className="dash-card" style={{ maxWidth: '600px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            <div className="dash-header" style={{ marginBottom: 8 }}>
            ...
            <div className="dash-item" style={{ display: 'grid', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Radicado:</label>
                  <input type="text" className="me-input" value={processData?.radicado || ''} onChange={(e) => setProcessData(prev => ({ ...prev, radicado: e.target.value }))} placeholder="110014105009-20250011400" style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Clase:</label>
                  <input type="text" className="me-input" value={processData?.clase || ''} onChange={(e) => setProcessData(prev => ({ ...prev, clase: e.target.value }))} placeholder="Laboral - Ordinario" style={{ width: '100%' }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Demandante:</label>
                  <input type="text" className="me-input" value={processData?.demandante || ''} onChange={(e) => setProcessData(prev => ({ ...prev, demandante: e.target.value }))} placeholder="Juan Pérez" style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Demandado:</label>
                  <input type="text" className="me-input" value={processData?.demandado || ''} onChange={(e) => setProcessData(prev => ({ ...prev, demandado: e.target.value }))} placeholder="Acme S.A.S." style={{ width: '100%' }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Juzgado:</label>
                  <input type="text" className="me-input" value={processData?.juzgado || ''} onChange={(e) => setProcessData(prev => ({ ...prev, juzgado: e.target.value }))} placeholder="JDO 009 MPC" style={{ width: '100%' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>Estado:</label>
                  <select className="me-select" value={processData?.estado || ''} onChange={(e) => setProcessData(prev => ({ ...prev, estado: e.target.value }))} style={{ width: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button className="btn btn-secondary" onClick={() => setShowProcessInfo(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={onSaveProcessInfo}>Guardar Información</button>
            </div>
          </div>
        </div>
      )}
        ...
        >
          <div className="dash-card" style={{ maxWidth: '500px', width: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="dash-item" style={{ display: 'grid', gap: 16 }}>
              ...
              <div style={{ background: '#1e2a3a', padding: '12px', borderRadius: '6px', color: '#e5edf7', fontSize: '14px', maxHeight: '200px', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              <button className="btn btn-secondary" onClick={() => setShowDeleteConfirm(false)} disabled={deletingItems}>Cancelar</button>
              <button className="btn btn-danger" onClick={handleDeleteSelected} disabled={deletingItems}>
      ...
      {isModal && showAudienceModal && (
        ...
        >
          <div
            style={{ background: '#0f172a', color: '#e2e8f0', width: '100%', maxWidth: 560, borderRadius: 14, padding: 16, boxShadow: '0 10px 32px rgba(0,0,0,0.45)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ margin: 0, fontSize: '18px', fontWeight: '600' }}>Programar Audiencia</h3>
              <button className="btn btn-secondary btn-sm" onClick={() => setShowAudienceModal(false)} style={{ padding: '4px 8px' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gap: 16 }}>
              {[
                { label: 'Fecha de Audiencia', key: 'fecha', type: 'datetime-local', placeholder: '' },
                { label: 'Actuación', key: 'actuacion', type: 'text', placeholder: 'Ej: Audiencia de conciliación' },
                { label: 'Juzgado', key: 'juzgado', type: 'text', placeholder: 'Ej: Juzgado Primero Civil del Circuito' },
              ].map(({ label, key, type, placeholder }) => (
                <div key={key}>
                  <label style={{ display: 'block', marginBottom: 6, fontSize: '14px', fontWeight: '500' }}>{label}</label>
                  <input
                    type={type}
                    value={audienceData[key]}
                    onChange={(e) => setAudienceData(prev => ({ ...prev, [key]: e.target.value }))}
                    placeholder={placeholder}
                    style={{ width: '100%', padding: '8px 12px', background: '#1b263b', color: '#e2e8f0', border: '1px solid rgba(148,163,184,0.35)', borderRadius: 8, fontSize: '14px' }}
                  />
                </div>
              ))}
              <div>
                <label style={{ display: 'block', marginBottom: 6, fontSize: '14px', fontWeight: '500' }}>Tipo de Audiencia</label>
                <select value={audienceData.tipo} onChange={(e) => setAudienceData(prev => ({ ...prev, tipo: e.target.value }))} style={{ width: '100%', padding: '8px 12px', background: '#1b263b', color: '#e2e8f0', border: '1px solid rgba(148,163,184,0.35)', borderRadius: 8, fontSize: '14px' }}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: 6, fontSize: '14px', fontWeight: '500' }}>Estado</label>
                <select value={audienceData.estado} onChange={(e) => setAudienceData(prev => ({ ...prev, estado: e.target.value }))} style={{ width: '100%', padding: '8px 12px', background: '#1b263b', color: '#e2e8f0', border: '1px solid rgba(148,163,184,0.35)', borderRadius: 8, fontSize: '14px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 }}>
              <button className="btn btn-secondary" onClick={() => setShowAudienceModal(false)}>Cancelar</button>
              <button
                className="btn btn-primary"
                onClick={() => {
                  setAudiences(prev => [...prev, { ...audienceData, id: Date.now() }]);
                  setShowAudienceModal(false);
                  setAudienceData({ fecha: '', actuacion: '', tipo: '', juzgado: '', estado: '' });
                }}
      ...
      {previewDoc && (
        <div
          role="dialog"
          aria-modal="true"
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 9000 }}
          onClick={(e) => { if (e.target === e.currentTarget) closePreviewModal(); }}
        >
          <div
            className="dash-card"
            style={{ width: '100%', maxWidth: 1100, maxHeight: '95vh', display: 'flex', flexDirection: 'column', background: '#0f172a', border: '1px solid rgba(148,163,184,0.35)', boxShadow: '0 25px 45px rgba(0,0,0,0.45)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="dash-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div>
                <div className="dash-title" style={{ marginBottom: 4 }}>Previsualizar documento</div>
                <div style={{ fontSize: 14, color: '#cbd5e1' }}>
                  {previewDoc?.name || previewDoc?.key?.split('/').pop() || 'Documento'}
                </div>
              </div>
              <button className="btn btn-secondary btn-sm" onClick={closePreviewModal}>Cerrar</button>
            </div>
            <div className="dash-item" style={{ flex: 1, overflow: 'hidden', background: '#0f172a' }}>
              {previewLoading ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '70vh', color: '#cbd5e1', gap: 12 }}>
                  <div style={{ width: 40, height: 40, border: '3px solid #4fd1c5', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '70vh', color: '#fecaca', textAlign: 'center', gap: 16 }}>
                  <div style={{ fontSize: 16 }}>{previewError || 'No se pudo cargar el documento.'}</div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                    <button className="btn btn-primary btn-sm" onClick={retryPreviewDocument} disabled={previewLoading}>Reintentar</button>
                    {previewDoc?.fallbackUrl && (
                      <button className="btn btn-secondary btn-sm" onClick={() => window.open(previewDoc.fallbackUrl, '_blank', 'noopener,noreferrer')}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              <button className="btn btn-secondary btn-sm" style={actionChipSecondary} onClick={() => onDownloadDocument(previewDoc, getFallbackUrl(previewDoc))} disabled={previewLoading}>⬇️ Descargar</button>
              <button className="btn btn-secondary btn-sm" style={actionChipSecondary} onClick={() => onShareDocument(previewDoc, getFallbackUrl(previewDoc))} disabled={previewLoading}>🔗 Compartir</button>
              {previewUrl && (
                <button className="btn btn-primary btn-sm" style={actionChipPrimary} onClick={() => window.open(previewUrl, '_blank', 'noopener,noreferrer')}>Abrir pestaña</button>
              )}
              <button className="btn btn-secondary btn-sm" style={actionChipSecondary} onClick={closePreviewModal}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isModal && showDeleteFolderModal && folderToDelete && (
        <div
          style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }}
          onClick={() => { if (!deletingFolder) { setShowDeleteFolderModal(false); setFolderToDelete(null); } }}
        >
          <div
            style={{ backgroundColor: '#1e2a3a', borderRadius: '12px', padding: '24px', maxWidth: '400px', width: '90%', border: '1px solid #394b61', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}
            onClick={(e) => e.stopPropagation()}
          >
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ color: '#e2e8f0', margin: '0 0 8px 0', fontSize: '18px', fontWeight: '600' }}>Eliminar Carpeta</h3>
              <p style={{ color: '#9fb3cc', margin: 0, fontSize: '14px', lineHeight: '1.5' }}>
                ¿Estás seguro de que quieres eliminar la carpeta <strong style={{ color: '#fc771c' }}>"{folderToDelete.name}"</strong>?
              </p>
              <p style={{ color: '#ef4444', margin: '8px 0 0 0', fontSize: '12px', fontWeight: '500' }}>
                Solo se puede eliminar si la carpeta está vacía.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '20px' }}>
              <button className="btn btn-secondary" onClick={() => { setShowDeleteFolderModal(false); setFolderToDelete(null); }} disabled={deletingFolder} style={{ padding: '8px 16px' }}>Cancelar</button>
              <button className="btn btn-danger" onClick={handleDeleteFolder} disabled={deletingFolder} style={{ padding: '8px 16px' }}>
